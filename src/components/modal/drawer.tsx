...
>(({ className, children, ...props }, ref) => (
  <DrawerPortal>
    ...
    <DrawerPrimitive.Content
      ref={ref}
      className={cn(
        "fixed inset-x-0 bottom-0 z-50 mt-24 h-auto rounded-t-[20px] border bg-background",
        className
      )}
      {...props}
    >
...
const DrawerHeader = ({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
  <div className={cn("w-full flex items-center justify-between px-3 py-4", className)} {...props} />
);
